...
def onko_bingo(bingoruudukot: list, bingonumerot: list):
    pelatut_numerot = []
    viimeinen_pelattu = None
    voittaneet = []
    bingot = bingoruudukot
    for numero in bingonumerot:
        pelatut_numerot.append(numero)
        for bingoruudukko in bingot:
            if ruudukko_oikein(bingoruudukko, pelatut_numerot):
                if bingoruudukko not in voittaneet:
                    viimeiset_pelatut = []
                    voittaneet.append(bingoruudukko)
                    viimeinen_pelattu = numero
                    for luku in pelatut_numerot:
                        viimeiset_pelatut.append(luku)
    print(summa(voittaneet[len(voittaneet)-1], viimeiset_pelatut, viimeinen_pelattu))
